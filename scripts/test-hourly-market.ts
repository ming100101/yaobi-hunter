...
const nowMs = utc + 2 * 3600_000 + 1;
// Coin candle timestamps use the runtime display offset. Derive it instead of
// assuming the developer machine is UTC+8 so this checks the same contract on CI.
...
const hourly = completedHourlyFromFiveMinute(coin, nowMs);
...
assert.equal(completedHourlyFromFiveMinute(gapped, nowMs).candles.length, 1, 'an incomplete clock hour is discarded');
