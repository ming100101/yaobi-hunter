// S1 checks:
//  (1) getSpotTickers hits the live OKX host and returns a populated map.
...
import { getSpotTickers } from '../src/data/okx';
...
// (1) live fetch
const map = await getSpotTickers('https://www.okx.com');
...
const withSpot = buildScanRecord([base({ spotVol24h: 361406323, basisPct: -0.05 })], 1_783_000_000_000, 'okx').coins[0];
ok('idx 19 spotVol24h rounded', withSpot[19] === 361406323, withSpot[19]);
ok('idx 20 basisPct (3dp)', withSpot[20] === -0.05, withSpot[20]);

const noSpot = buildScanRecord([base({})], 1_783_000_000_000, 'okx').coins[0];
